...
try {
    if ($role) {
        // Check if any approved entries exist for this role
        $stmt = $conn->prepare("SELECT COUNT(*) FROM check_ins WHERE status = 'approved' AND role = :role");
        ...
        if ($roleExists) {
            ...
            $stmt = $conn->prepare("SELECT id, code, full_name, selfie_path, visit_purpose, created_at FROM check_ins WHERE status = 'approved' AND role = :role");
            $stmt->bindParam(':role', $role, PDO::PARAM_STR);
        } else {
            // If no matching role entries exist, fetch all approved entries
            $stmt = $conn->prepare("SELECT id, code, full_name, selfie_path, visit_purpose, created_at FROM check_ins WHERE status = 'approved'");
        }
    } else {
        // Fetch all approved entries if no role is provided
        $stmt = $conn->prepare("SELECT id, code, full_name, selfie_path, visit_purpose, created_at FROM check_ins WHERE status = 'approved'");
    }
